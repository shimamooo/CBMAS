...
def get_choice_token_ids(model) -> Tuple[int, int]:
    """Return token ids for 'A' and 'B' tokens (simple numbers) used in reassurance dataset."""
    choice1_id = int(model.to_tokens("A", prepend_bos=False)[0, 0])
    choice2_id = int(model.to_tokens("B", prepend_bos=False)[0, 0])
